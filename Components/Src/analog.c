...
void Analog_Init(Analog_Handle_t *comp_handle) {
  ...
  HAL_ADC_Start_DMA(handle->hadc, (uint32_t*)handle->raw_data_adc, ADC_CHANNELS * ADC_SAMPLES);
 ...
 */
static float process_channel(float raw_val, SensorType_t type, uint8_t led_num) {
  float raw_norm = raw_val / 4095.0f;
...
void Analog_GetValues(ProjectVars_t *project_vars) {
  // Обработка 4 каналов АЦП (и индикация на LED 1-4)
  for (int i = 0; i < 4; i++) {
  ...
  for (int ch = 0; ch < ADC_CHANNELS; ch++) {
          ...
          // Собираем каждое 4-е значение (смещение для конкретного канала)
          for (int s = 0; s < ADC_SAMPLES; s++) {
              sum += handle->raw_data_adc[s * ADC_CHANNELS + ch];
          }

          float avg_raw = (float)sum / (float)ADC_SAMPLES;

          project_vars->vars[ain_ch0_id + ch].as_float = process_channel(
              avg_raw, // Передаем уже усредненный float
              handle->channel_types_adc[ch],
              ch + 1
          );
      }
}
